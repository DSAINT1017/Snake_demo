...
class Snake_Game(Env):
    ...
    def __init__(self, mode="test"):
        ...
        self.observation_space = spaces.Box(low = 0, high = 4, shape = (8, 8))
    ...
    def initial_each_step(self):
        ...
        if self.term == 0:
            ...
            #location_row = np.random.randint(low=1, high=8)
            #location_column = np.random.randint(low=1, high=6)
    ...
    #每一回合的執行（包括選擇動作、更新現有資產情況、計算reward等等）
    def step(self, action):
        ...
        if action%2 == 0:
            ...
            if row < 8 and row >= 0:
                ...
                return self.get_observation(), -30, self.DONE, info
...
if mode == 'train':
    ...
    #讀取權重
    weights = "dqn_"+Snake_env.name+"_weights_" + str(step) + ".h5f"
    #weights = "dqn_Snake_weights_9999000.h5f"
    if weights:
        weights_filename = weights
    dqn.load_weights(weights_filename)
    dqn.test(Snake_env, nb_episodes=3, visualize=True)
